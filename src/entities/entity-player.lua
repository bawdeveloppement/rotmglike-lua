...
function Player:drawQuickSlots ()
    ...
    for i, v in ipairs(self.quickSlots) do
        ...
        if self.quickSlots[i].item ~= nil then
            ...
            love.graphics.print(""..self.quickSlots[i].quantity, slotx, sloty)
            ...
            if mx > slotx and mx < slotx + 32 and my > sloty and my < sloty + 32 then
                ...
                local itemId = item.id or (""..#self.quickSlots..love.math.random(0, 100))
                print(itemId)
...
function Player:drawItemInMouse()
    if self.itemInMouse then
        ...
        if self.itemInMouse.item ~= nil then
            ...
            elseif self.itemInMouse.item.AnimatedTexture ~= nil then
                local image = _G.xle.ResourcesManager:getTexture(self.itemInMouse.item.AnimatedTexture.File);
                if image ~= nil then
                    local imageW, imageH = image:getDimensions()
                    local quad = love.graphics.newQuad(
                        8 * (self.itemInMouse.item.AnimatedTexture.Index % (imageW / 8)),
                        math.floor(self.itemInMouse.item.AnimatedTexture.Index / (imageW / 8)) * 8,
                        8,
                        8,
                        imageW, imageH
                    )
